...
export type {
  BachsPlatformBalance,
  BachsCreateTransferOpts,
  BachsAccountCapabilities,
  BachsCapabilityEntry,
  BachsCreateRefundOpts,
  BachsRefundResult,
} from './bachs'
...
export function getActivePaymentProvider(): PaymentProvider {
  ...
  if (!SUPPORTED.has(active)) {
    throw new Error(`Unsupported payment provider: "${active}". Supported: ${[...SUPPORTED].join(', ')}`)
  }
  ...
  switch (active) {
    ...
    case 'bachs': {
      ...
      if (!key) {
        throw new Error('BACHS_TEST_KEY or BACHS_LIVE_KEY is required when ACTIVE_PAYMENT_PROVIDER=bachs')
      }
